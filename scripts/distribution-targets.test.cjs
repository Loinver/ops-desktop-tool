...
test('distribution is limited to macOS and Windows without implicit publishing', () => {
  ...
  assert.match(releaseJob, /- name: Remove legacy dot-named release assets/)
  assert.match(releaseJob, /Ops\.Desktop-\$\{version\}-x64\.dmg/)
...
test('release assets bypass Actions artifact quota and publish through a draft release', () => {
  ...
  assert.match(macBuildJob, /release\/Ops Desktop-\$\{version\}-\$\{\{ matrix\.arch \}\}\.dmg/)
  assert.match(macBuildJob, /release\/Ops Desktop-\$\{version\}-\$\{\{ matrix\.arch \}\}\.zip/)
  ...
  assert.match(winBuildJob, /Ops Desktop-\$version-windows-\$\{\{ matrix\.arch \}\}\.exe/)
  assert.match(winBuildJob, /Ops Desktop-\$version-windows-\$\{\{ matrix\.arch \}\}\.zip/)
