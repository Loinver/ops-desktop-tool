...
const macBuildJob = workflow.slice(workflow.indexOf('  mac-build:'), workflow.indexOf('  win-build:'))
...
test('tag pushes are the only release trigger and publish source', () => {
  ...
  assert.doesNotMatch(workflow, /github\.event_name\s*={1,2}\s*['"]release['"]|github\.event\.release\.tag_name/)

  assert.match(macBuildJob, /- name: Build unsigned Mac package for CI\n        if: \$\{\{ !startsWith\(github\.ref, 'refs\/tags\/v'\) \}\}/)
  for (const stepName of [
    'Validate Mac release credentials',
    'Build signed and notarized Mac package',
    'Verify signed and notarized Mac app'
  ]) {
    const stepStart = macBuildJob.indexOf(`- name: ${stepName}`)
    const nextStepStart = macBuildJob.indexOf('\n      - name:', stepStart + 1)
    const step = macBuildJob.slice(stepStart, nextStepStart === -1 ? undefined : nextStepStart)
    assert.match(step, /if: startsWith\(github\.ref, 'refs\/tags\/v'\)/)
  }
