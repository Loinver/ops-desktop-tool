...
| 实验功能  | AI 图像实验  | OpenAI 兼容文生图、编辑/变体、1–4 张批量、取消/重试、本地历史、费用估算、单批上限及每日/月度预算保护                                    |
...
AI 对话还可添加最多 4 张 PNG、JPG/JPEG 或 WEBP 故障截图。Renderer 只提交临时证据标识，主进程才读取用户通过系统选择器选中的文件，并进行文件头、原始大小、像素尺寸与总量、缩放和规范化大小校验；原始路径、原始字节和图片 Base64 不会返回 Renderer 或写入对话历史。证据按 Renderer 隔离、30 分钟过期，并会在成功发送、手动清空或窗口销毁后清除。图片内容可能包含伪造指令或敏感信息，模型只应将其作为待核实的观察材料；图片请求的费用会按“未知多模态费用”进入保护性预算判断，仍以 Provider 实际账单为准。

## AI 图像实验

AI 图像实验支持 OpenAI 兼容的文生图、基于本地生成图的编辑/变体、1–4 张批量、停止生成、临时错误自动重试和原消息手动重试。生成原图由主进程写入受限资源目录，Renderer 只接收资源标识和小尺寸预览。

页面会显示下一批、今日和本月费用估算，并可配置兼容 Provider 的每张费用、单批费用上限，以及与 AI 对话共用的每日/月度预算。主进程会把最大重试次数计入请求前预留，并阻止并发请求穿透预算；超限只允许对当前 Renderer 和当前请求参数执行一次手动继续。费用均为本地保护性估算，不替代 Provider 账单，正式发布前仍需使用真实 Provider 返回校准。
...
3. 首次运行时配置 AI Provider（OpenAI 兼容、Anthropic 或 Gemini）和 SFTP（可选）
