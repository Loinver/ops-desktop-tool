...
| 总览 | 运维中心 | 集中处理运维事件、运行 HTTP/TCP 自动化巡检，并使用 AI Copilot 辅助排障 |
...
模型可靠性测试完成后会保存最小化结果快照（状态、耗时、HTTP 状态与诊断，不保存 API Key 和真实回复正文）。当前测试范围可一键设为巡检目标；Electron 主进程按配置间隔执行巡检，失败时可发送系统桌面通知。首页运维仪表盘展示最近发布、模型可用率与趋势。
