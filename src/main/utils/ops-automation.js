...
function eventsPath(userDataPath) {
  return path.join(userDataPath, 'ops-events.json')
}
function tasksPath(userDataPath) {
  return path.join(userDataPath, 'ops-automation-tasks.json')
}
...
function normalizeTimeline(input) {
  ...
  return input.slice(-MAX_EVENT_TIMELINE).map((item) => ({
    ...
    type: ['opened', 'occurred', 'reopened', 'acknowledged', 'resolved', 'recovered'].includes(
      item?.type
    )
      ? item.type
      : 'occurred',
...
function normalizeStoredEvent(input = {}) {
  ...
  const sourceId = value(input.sourceId || input.relatedId, 180)
  const fingerprint =
    !input.fingerprint && sourceType === 'automation' && sourceId
      ? `automation:${sourceId}`.slice(0, 240)
      : eventFingerprint({ ...input, sourceType, sourceId })
  const firstOccurredAt =
    Number(input.firstOccurredAt || input.createdAt || input.updatedAt) || Date.now()
  const lastOccurredAt =
    Number(input.lastOccurredAt || input.updatedAt || input.createdAt) || firstOccurredAt
...
function loadEventState(userDataPath) {
  ...
  const normalizedItems = Array.isArray(stored?.items)
    ? stored.items.slice(0, MAX_EVENTS).map((item) =>
        normalizeStoredEvent({
          ...item,
          readAt:
            item?.readAt ??
            (storedVersion < EVENT_STATE_VERSION &&
            normalizeEventStatus(item?.status) === 'resolved'
...
function saveEventState(userDataPath, state) {
  ...
  if (!writeJsonFile(eventsPath(userDataPath), { version: EVENT_STATE_VERSION, items }))
    throw new Error('保存运维事件失败')
}
function addOpsEvent(userDataPath, input = {}) {
  ...
  const sourceType =
    value(input.sourceType || input.category || existing?.sourceType, 60) || 'system'
  ...
  const status =
    existing && requestedStatus === 'open' && existing.status !== 'resolved'
      ? existing.status
      : requestedStatus
  const timelineType = existing
...
function updateOpsEvent(userDataPath, id, status, note = '') {
  ...
  item.acknowledgedAt =
    nextStatus === 'acknowledged' ? item.acknowledgedAt || updatedAt : item.acknowledgedAt
...
function recoverOpsEvent(userDataPath, fingerprint, input = {}) {
  ...
  const item = state.items.find(
    (entry) => entry.fingerprint === value(fingerprint, 240) && entry.status !== 'resolved'
  )
  if (!item) return null
...
function listOpsEvents(
  ...
  return loadEventState(userDataPath)
    .items.filter((item) => !normalizedStatus || item.status === normalizedStatus)
    ...
    .filter(
      (item) =>
        !normalizedQuery ||
        `${item.title} ${item.description} ${item.sourceType} ${item.sourceId}`
          .toLowerCase()
...
function normalizeTask(input = {}, existing = {}) {
  const type = ['http-health', 'tcp-port'].includes(input.type ?? existing.type)
  ...
  const port =
    type === 'tcp-port' ? Math.max(1, Math.min(65535, Number(input.port ?? existing.port) || 0)) : 0
  if (type === 'tcp-port' && !port) throw new Error('请输入有效端口')
  const intervalMinutes = Math.max(
    MIN_INTERVAL_MINUTES,
    Math.min(MAX_INTERVAL_MINUTES, Number(input.intervalMinutes ?? existing.intervalMinutes) || 15)
  )
  ...
  return {
    ...
    title:
      value(input.title ?? existing.title, 120) ||
      (type === 'http-health' ? 'HTTP 健康检查' : 'TCP 端口检查'),
    type,
    target,
    port,
    expectedStatus: Math.max(
      100,
      Math.min(599, Number(input.expectedStatus ?? existing.expectedStatus) || 200)
    ),
    timeoutMs: Math.max(
      1000,
      Math.min(60_000, Number(input.timeoutMs ?? existing.timeoutMs) || 8000)
    ),
    intervalMinutes,
    enabled,
...
function saveAutomationState(userDataPath, state) {
  if (
    !writeJsonFile(tasksPath(userDataPath), { version: 1, tasks: state.tasks.slice(0, MAX_TASKS) })
  )
    throw new Error('保存自动化任务失败')
}
function listAutomationTasks(userDataPath) {
  return loadAutomationState(userDataPath).tasks
}
...
async function runHttpHealthCheck(task) {
  ...
  try {
    const response = await fetch(task.target, {
      method: 'GET',
      redirect: 'manual',
      signal: controller.signal,
      headers: { 'user-agent': 'Ops-Desktop-Health-Check/1.0' }
    })
    ...
    return {
      ...
      message: ok
        ...
        : `期望 HTTP ${task.expectedStatus}，实际 HTTP ${response.status}`
    }
  } catch (error) {
    return {
      ok: false,
      ...
      message:
        error?.name === 'AbortError'
          ? `请求超时（${task.timeoutMs}ms）`
          : value(error?.message, 500) || '健康检查失败'
    }
  } finally {
    clearTimeout(timer)
  }
...
async function runAutomationTask(userDataPath, id) {
  ...
  if (!task) throw new Error('自动化任务不存在')
  const result =
    task.type === 'tcp-port' ? await runTcpPortCheck(task) : await runHttpHealthCheck(task)
  const completedAt = Date.now()
  const entry = {
    id: crypto.randomUUID(),
    ...result,
    startedAt: completedAt - result.durationMs,
...
async function runDueAutomationTasks(userDataPath) {
  ...
  for (const task of tasks.slice(0, 10)) {
    try {
      results.push(await runAutomationTask(userDataPath, task.id))
    } catch (error) {
      results.push({ task, result: { ok: false, message: value(error?.message, 500) } })
    }
  }
  return results
}
