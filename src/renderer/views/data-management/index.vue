  ...
  <div class="page data-management-page">
    <header class="page-header">
      <div class="page-heading">
        ...
        <p class="page-desc">
          按功能分类导出加密备份；恢复前会先校验内容，并为当前数据保留本机恢复点。
        </p>
      </div>
    ...
    <main class="page-content">
      <section class="data-summary" aria-label="本地数据概览">
        ...
        <article
          class="surface-panel data-summary-card"
          :class="{ 'data-summary-card--warning': invalidFiles.length }"
        >
          <span>数据校验</span>
          <strong>{{ invalidFiles.length ? '需处理' : '正常' }}</strong>
          <small>{{
            invalidFiles.length
              ? `${invalidFiles.length} 个文件无法纳入备份`
              : '导出前将再次验证内容'
          }}</small>
        </article>
      </section>
      ...
      <section class="surface-panel page-section" aria-labelledby="backup-title">
        ...
        <div v-if="invalidFiles.length" class="validation-note validation-note--warning">
          <t-icon name="error-circle" /> 以下文件不是有效 JSON，已暂时排除：{{
            invalidFiles.join('、')
          }}。
        </div>
        ...
        <div class="backup-group-grid">
          <label
            ...
            :key="group.id"
            :class="['backup-group', { 'is-disabled': !group.available }]"
          >
            <input
            ...
            <span class="backup-group-copy">
              ...
              <em v-if="group.available"
                >{{ group.fileCount }} 个文件 · {{ formatBytes(group.sizeBytes) }}</em
              >
        ...
        <div class="panel-actions">
          ...
          >
            <t-icon
              :name="busyAction === 'export' ? 'loading' : 'download'"
              :class="{ spinning: busyAction === 'export' }"
            />
            导出加密备份
          </button>
        </div>
      ...
      <section class="surface-panel page-section" aria-labelledby="restore-title">
        <div class="section-heading">
          <div>
            ...
            <p class="section-desc">
              恢复只替换备份中存在的数据文件，未包含的数据保持不变；完成后需要重启应用以重新加载服务和页面状态。
            </p>
          </div>
        </div>
        ...
        <div class="restore-controls">
          <label class="field-label restore-password">
            ...
            <input
              ...
              minlength="8"
              maxlength="256"
              placeholder="输入所选备份的密码"
              @keyup.enter="inspectBackup"
            />
          </label>
          <button
            class="btn-secondary"
          ...
          >
            <t-icon
              :name="busyAction === 'inspect' ? 'loading' : 'file-search'"
              :class="{ spinning: busyAction === 'inspect' }"
            />
            选择并校验备份
          </button>
        </div>
        ...
        <div v-if="backupPreview" class="restore-preview">
          <div class="restore-preview-heading">
            ...
            <span class="preview-time"
              >导出于 {{ formatDate(backupPreview.summary.createdAt) }}</span
            >
          </div>
          <dl class="preview-metrics">
            <div>
              <dt>来源版本</dt>
              <dd>{{ backupPreview.summary.appVersion || '未知版本' }}</dd>
            </div>
            <div>
              <dt>数据文件</dt>
              <dd>{{ backupPreview.summary.fileCount }} 个</dd>
            </div>
            <div>
              <dt>恢复体积</dt>
              <dd>{{ formatBytes(backupPreview.summary.sizeBytes) }}</dd>
            </div>
          </dl>
          <div class="preview-groups">
            <span v-for="group in backupPreview.summary.groups" :key="group.id"
              >{{ group.label }} · {{ group.fileCount }} 个</span
            >
          </div>
          <div class="panel-actions panel-actions--restore">
            <p>
              <t-icon name="info-circle" /> 恢复前会保存当前同名文件的本机恢复点，仅保留最近 3 次。
            </p>
            <button class="btn-danger" type="button" :disabled="busy" @click="restoreBackup">
              <t-icon
                :name="busyAction === 'restore' ? 'loading' : 'upload'"
                :class="{ spinning: busyAction === 'restore' }"
              />
              恢复并重启
            </button>
          </div>
        ...
        <div v-else class="restore-empty">
          <t-icon name="file" /> 选择备份并输入密码后，可在恢复前查看其中的分类、文件数和导出时间。
        </div>
      </section>
      ...
      <section class="surface-panel page-section" aria-labelledby="auto-backup-title">
        <div class="section-heading">
          <div>
            ...
            <p class="section-desc">
              主进程会按计划生成加密备份；密码仅使用系统安全存储保存，不会回显到页面。
            </p>
          </div>
          <span class="section-status" :class="{ 'section-status--muted': !autoSettings.enabled }">
            <t-icon :name="autoSettings.enabled ? 'check-circle' : 'stop-circle'" />
            {{ autoSettings.enabled ? '计划已启用' : '计划未启用' }}
          </span>
        </div>
        ...
        <div class="auto-settings-grid">
          <label class="toggle-field">
            ...
            <span
              ...
              ><small>关闭后不会执行计划任务，也不会删除已有备份。</small></span
            >
          </label>
          <label class="field-label">
            <span>执行频率</span>
            <select v-model="autoSettings.interval" class="field-input">
          ...
          <label class="field-label">
            <span>保留数量</span>
            <input
              v-model.number="autoSettings.retentionCount"
              class="field-input"
              type="number"
              min="1"
              max="30"
          ...
          <label class="field-label auto-directory-field">
            ...
            <div class="path-control">
              <input
                :value="autoSettings.outputDirectory"
                class="field-input"
                type="text"
                readonly
                placeholder="选择专用目录保存 .opsbackup 文件"
              /><button
                class="btn-secondary"
                type="button"
                :disabled="busy"
                @click="chooseAutoBackupDirectory"
              >
                <t-icon name="folder-open" /> 选择目录
              </button>
            </div>
          </label>
          <label class="field-label">
            ...
            <input
              ...
              maxlength="256"
              :placeholder="
                autoSettings.hasPassword ? '已安全保存；留空则继续使用' : '首次启用时至少 8 个字符'
              "
            />
          </label>
        </div>
        ...
        <div class="backup-group-grid backup-group-grid--auto">
          <label v-for="group in overview.groups" :key="`auto-${group.id}`" class="backup-group">
            ...
            <span class="backup-group-copy"
              ><strong>{{ group.label }}</strong
              ><small>{{ group.description }}</small
              ><em>{{ group.fileCount }} 个当前数据文件</em></span
            >
          </label>
        </div>
        ...
        <div class="auto-status-line">
          <span
            >上次执行：{{
              autoSettings.lastRunAt ? formatDate(autoSettings.lastRunAt) : '尚未执行'
            }}</span
          >
          <span
            >下次执行：{{
              autoSettings.enabled && autoSettings.nextRunAt
                ? formatDate(autoSettings.nextRunAt)
                : '保存并启用后生成计划'
            }}</span
          >
        ...
        <div class="auto-health" :class="`auto-health--${autoHealth.status}`" aria-live="polite">
          ...
          <div>
            <strong>{{ autoHealthLabel(autoHealth) }}</strong>
            <p>{{ autoHealth.summary }}</p>
          </div>
        ...
        <div class="panel-actions">
          ...
          <div class="panel-action-buttons">
            <button
              ...
              type="button"
              :disabled="busy"
              @click="checkAutoBackupHealth"
            >
              <t-icon
                :name="busyAction === 'auto-health' ? 'loading' : 'check-circle'"
                :class="{ spinning: busyAction === 'auto-health' }"
              />
              检查健康度
            </button>
            <button class="btn-secondary" type="button" :disabled="busy" @click="saveAutoBackup">
              <t-icon
                :name="busyAction === 'auto-save' ? 'loading' : 'save'"
                :class="{ spinning: busyAction === 'auto-save' }"
              />
              保存计划
            </button>
            <button
              class="btn-primary"
              type="button"
              :disabled="busy || !canRunAutoBackup"
              @click="runAutoBackup"
            >
              <t-icon
                :name="busyAction === 'auto-run' ? 'loading' : 'play-circle'"
                :class="{ spinning: busyAction === 'auto-run' }"
              />
              立即备份
            </button>
          </div>
        ...
        <div class="recovery-grid">
          <div class="recovery-column">
            <div class="subsection-heading">
              <strong>自动备份历史</strong><span>最近 {{ autoHistory.length }} 条</span>
            </div>
            <div v-if="autoHistory.length" class="history-list">
              <article
                v-for="item in autoHistory"
                :key="item.id"
                class="history-item history-item--auto"
                :class="{
                  'history-item--failed': item.status === 'failed',
                  'history-item--missing': item.availability === 'missing'
                }"
              >
                <div>
                  <strong>{{ item.status === 'success' ? item.fileName : '自动备份失败' }}</strong
                  ><small
                    >{{ formatDate(item.createdAt) }} ·
                    {{
                      item.status === 'success' ? formatBytes(item.sizeBytes) : item.error
                    }}</small
                  >
                </div>
                <div class="history-item-side">
                  <span :class="{ 'history-status--missing': item.availability === 'missing' }">{{
                    autoBackupStateLabel(item)
                  }}</span>
                  <div v-if="item.status === 'success'" class="history-item-actions">
                    <button
                      ...
                      type="button"
                      :disabled="busy"
                      @click="openAutoBackupDirectory(item)"
                    >
                      <t-icon name="folder-open" /> 目录
                    </button>
                    <button
                      ...
                      type="button"
                      :disabled="busy"
                      @click="inspectAutoHistoryBackup(item)"
                    >
                      <t-icon
                        :name="busyAction === `auto-inspect-${item.id}` ? 'loading' : 'file-search'"
                        :class="{ spinning: busyAction === `auto-inspect-${item.id}` }"
                      />
                      校验
                    </button>
                    <button
                      ...
                      type="button"
                      :disabled="busy"
                      @click="restoreAutoHistoryBackup(item)"
                    >
                      <t-icon
                        :name="busyAction === `auto-restore-${item.id}` ? 'loading' : 'rollback'"
                        :class="{ spinning: busyAction === `auto-restore-${item.id}` }"
                      />
                      恢复
                    </button>
                    <button
                      class="btn-danger btn-danger--compact"
                      type="button"
                      :disabled="busy"
                      @click="deleteAutoHistoryBackup(item)"
                    >
                      <t-icon
                        :name="busyAction === `auto-delete-${item.id}` ? 'loading' : 'delete'"
                        :class="{ spinning: busyAction === `auto-delete-${item.id}` }"
                      />
                      {{ item.availability === 'missing' ? '清理记录' : '删除' }}
                    </button>
                  </div>
                </div>
              </article>
          ...
          <div class="recovery-column">
            ...
            <div v-if="restorePoints.length" class="history-list">
              ...
              >
                ...
                <button
                  ...
                  type="button"
                  :disabled="busy"
                  @click="restorePoint(point)"
                >
                  <t-icon
                    :name="busyAction === `restore-point-${point.id}` ? 'loading' : 'rollback'"
                    :class="{ spinning: busyAction === `restore-point-${point.id}` }"
                  />
                  回滚
                </button>
              </article>
            ...
            <div v-else class="restore-empty">
              <t-icon name="file" /> 暂无恢复点；恢复外部备份前会自动创建。
            </div>
          </div>
        </div>
      </section>
      ...
      <section class="data-security-note" aria-label="备份说明">
        ...
        <div>
          <strong>安全说明</strong>
          <p>
            备份文件已由你的密码加密。已保存的 SFTP 与 AI
            凭证仍使用系统安全存储保护；跨设备恢复后，如系统安全存储无法解密旧凭证，请在对应页面重新输入密码或
            API Key。
          </p>
        </div>
      </section>
...
const availableGroupCount = computed(
  () => overview.value.groups.filter((group) => group.available).length
)
const totalFiles = computed(() =>
  overview.value.groups.reduce((total, group) => total + group.fileCount, 0)
)
const totalSize = computed(() =>
  overview.value.groups.reduce((total, group) => total + group.sizeBytes, 0)
)
const invalidFiles = computed(() =>
  overview.value.groups.flatMap((group) => group.invalidFiles || [])
)
...
const canExport = computed(
  () =>
    ...
    backupPassword.value.length >= 8 &&
    backupPassword.value === backupPasswordConfirm.value
)
const canRunAutoBackup = computed(
  () =>
    autoSettings.value.enabled &&
    autoSettings.value.outputDirectory &&
    autoSettings.value.hasPassword &&
    autoSettings.value.categories.length > 0
)
...
async function loadOverview() {
  loading.value = true
  try {
    const next = await opsApi.getDataBackupOverview()
    overview.value = next || { groups: [] }
    const available =
      next?.groups?.filter((group) => group.available).map((group) => group.id) || []
    ...
    MessagePlugin.error({
      content: error.message || '无法读取本地数据概览',
...
async function exportBackup() {
  ...
  try {
    const result = await opsApi.exportDataBackup({
      categories: selectedGroups.value,
      password: backupPassword.value
    })
    if (!result?.canceled) {
      MessagePlugin.success({
        content: `已导出 ${result.fileName}（${formatBytes(result.sizeBytes)}）`,
        placement: 'bottom-right'
      })
...
async function inspectBackup() {
  ...
  try {
    const result = await opsApi.inspectDataBackup({ password: restorePassword.value })
    if (!result?.canceled) {
...
async function restoreBackup() {
  ...
  try {
    await opsApi.restoreDataBackup({ token: backupPreview.value.token })
    MessagePlugin.success({ content: '数据已恢复，正在重启应用…', placement: 'bottom-right' })
    await opsApi.relaunchApp()
  } catch (error) {
...
async function loadAutoBackupState() {
  try {
    ...
    MessagePlugin.error({
      content: error.message || '无法读取自动备份状态',
      placement: 'bottom-right'
    })
  }
}
...
async function loadRestorePoints() {
  try {
    const points = await opsApi.getDataRestorePoints()
    restorePoints.value = Array.isArray(points) ? points : []
  } catch (error) {
    MessagePlugin.error({
      content: error.message || '无法读取本机恢复点',
      placement: 'bottom-right'
    })
  }
}
...
async function chooseAutoBackupDirectory() {
  const directory = await opsApi.browseFile({
    directory: true,
    defaultPath: autoSettings.value.outputDirectory
  })
...
async function checkAutoBackupHealth() {
  ...
  try {
    const health = await opsApi.getAutoBackupHealth()
    autoHealth.value = { ...autoHealth.value, ...(health || {}) }
    const notify =
      autoHealth.value.status === 'error'
        ? MessagePlugin.error
        : autoHealth.value.status === 'warning'
          ? MessagePlugin.warning
          : MessagePlugin.success
    notify({ content: autoHealth.value.summary, placement: 'bottom-right' })
  } catch (error) {
    MessagePlugin.error({
      content: error.message || '自动备份健康检查失败',
      placement: 'bottom-right'
    })
  } finally {
    busyAction.value = ''
...
async function saveAutoBackup() {
  ...
  try {
    const result = await opsApi.saveAutoBackupSettings({
    ...
    autoPassword.value = ''
    MessagePlugin.success({
      content: autoSettings.value.enabled ? '自动备份计划已保存' : '自动备份计划已关闭',
      placement: 'bottom-right'
    })
    await loadAutoBackupState()
  } catch (error) {
    MessagePlugin.error({
      content: error.message || '保存自动备份计划失败',
      placement: 'bottom-right'
    })
  } finally {
    busyAction.value = ''
...
async function runAutoBackup() {
  ...
  try {
    const result = await opsApi.runAutoBackupNow()
    if (result?.settings) autoSettings.value = { ...autoSettings.value, ...result.settings }
    MessagePlugin.success({
      content: `已创建自动备份 ${result?.entry?.fileName || ''}`,
      placement: 'bottom-right'
    })
    await loadAutoBackupState()
  } catch (error) {
...
async function openAutoBackupDirectory(item) {
  ...
  try {
    await opsApi.openAutoBackupDirectory(item.id)
  } catch (error) {
    MessagePlugin.error({
      content: error.message || '无法打开自动备份目录',
      placement: 'bottom-right'
    })
  } finally {
    busyAction.value = ''
...
async function inspectAutoHistoryBackup(item) {
  ...
  try {
    const result = await opsApi.inspectAutoBackup(item.id)
    const summary = result?.summary
    MessagePlugin.success({
      content: `校验通过：${summary?.fileCount || 0} 个数据文件，${formatBytes(summary?.sizeBytes)}`,
      placement: 'bottom-right'
    })
  } catch (error) {
...
async function restoreAutoHistoryBackup(item) {
  ...
  try {
    await opsApi.restoreAutoBackup(item.id)
...
async function deleteAutoHistoryBackup(item) {
  ...
  try {
    const result = await opsApi.deleteAutoBackup(item.id)
    MessagePlugin.success({
      content: result?.deleted ? '自动备份文件及记录已删除' : '缺失的自动备份记录已清理',
      placement: 'bottom-right'
    })
    await loadAutoBackupState()
  } catch (error) {
...
async function restorePoint(point) {
  ...
  try {
    await opsApi.restoreDataRestorePoint(point.id)
...
.data-management-page :deep(.btn-danger) {
...
.data-management-page :deep(.btn-primary:hover:not(:disabled)) {
...
.data-management-page :deep(.btn-secondary:hover:not(:disabled)) {
...
.data-management-page :deep(.btn-danger:hover:not(:disabled)) {
  background: #dc2626;
  box-shadow: var(--shadow-xs);
}
...
.data-summary-card--warning strong {
  color: var(--warning);
}
.section-status {
  display: inline-flex;
...
.validation-note--warning {
  border: 1px solid color-mix(in srgb, var(--warning) 35%, var(--border-light));
...
.backup-group-copy strong,
.backup-group-copy small,
.backup-group-copy em {
  display: block;
}
.backup-group-copy strong {
  color: var(--text);
...
.restore-preview {
  margin-top: var(--spacing-lg);
  padding: 16px;
  border: 1px solid color-mix(in srgb, var(--success) 25%, var(--border-light));
  border-radius: var(--radius-md);
...
.preview-label,
.restore-preview-heading strong {
  display: block;
}
.preview-label {
  color: var(--success);
...
.data-security-note > .t-icon {
  flex: 0 0 auto;
  margin-top: 2px;
  color: var(--primary);
}
.data-security-note strong {
  color: var(--text);
  font-size: 13px;
}
.data-security-note p {
  margin-top: 3px;
  color: var(--text-muted);
  font-size: 12px;
  line-height: 19px;
...
.auto-health--healthy > .t-icon {
  color: var(--success);
}
.auto-health--warning {
  border-color: color-mix(in srgb, var(--warning) 32%, var(--border-light));
  background: color-mix(in srgb, var(--warning) 7%, var(--card-bg));
}
.auto-health--warning > .t-icon {
  color: var(--warning);
}
.auto-health--error {
...
.history-item > .history-item-side {
  display: flex;
  align-items: center;
...
.history-item--failed .history-item-side > span {
  color: var(--danger);
}
.history-item-side > .history-status--missing {
  color: var(--warning);
}
...
.history-item-actions .btn-danger--compact {
  border-color: color-mix(in srgb, var(--danger) 48%, var(--border));
  background: var(--card-bg);
  color: var(--danger);
}
.history-item-actions .btn-danger--compact:hover:not(:disabled) {
  background: color-mix(in srgb, var(--danger) 8%, var(--card-bg));
  box-shadow: none;
}
.history-item--restore-point .btn-secondary {
  flex: 0 0 auto;
}
...
@media (max-width: 760px) {
  ...
  .restore-preview-heading,
  .history-item--auto,
  .history-item--auto > .history-item-side,
  .auto-health {
    align-items: stretch;
    flex-direction: column;
  }
  ...
  .history-item--auto .history-item-side,
  .history-item--auto .history-item-actions {
  ...
  .restore-controls > .btn-secondary {
    width: 100%;
    justify-content: center;
  }
}
