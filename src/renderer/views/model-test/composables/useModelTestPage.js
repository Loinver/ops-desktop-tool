...
import { useModelTestKeyboard } from './useModelTestKeyboard.js'
import { useModelTestPersistence } from './useModelTestPersistence.js'
import {
  CONCURRENCY,
  COPY_FIELD_LABELS,
  EMPTY_RESULT,
...
export function useModelTestPage() {
  ...
  } = useModelTestPersistence({ results, selectedScopeKeys })
  ...
  async function copyProviderValue(provider, field) {
    try {
      await opsApi.copyModelProviderValue({
  ...
  function fetchModels(provider, { force = false } = {}) {
    ...
    const request = scheduleModelLoad(async () => {
      try {
        ...
        const res = await opsApi.listProviderModels({
          providerId: provider.id,
          appType: provider.appType
        })
        if (generation !== modelLoadGeneration) return
  ...
  const modelListSettingsSummary = computed(() => {
    ...
    if (mode === 'include') {
      return includeRules.length ? `筛选 ${includeRules.length} 条规则` : '模型筛选（未设置规则）'
    }
  ...
  async function loadModelListSettings() {
    try {
      const res = await opsApi.getModelListSettings()
  ...
  async function saveModelListSettings(settings) {
    ...
    try {
      const res = await opsApi.saveModelListSettings(settings)
      if (!res?.ok || !res.settings) {
          ...
          const entries = group.entries
            .map((entry) => {
              const filteredRows = entry.rows.filter((row) => {
                ...
                if (query && !nameHit) {
                  const hay =
                    `${row.model} ${row.label || ''} ${entry.appLabel || ''}`.toLowerCase()
  ...
  async function reload() {
    ...
    try {
      const res = await opsApi.listModelProviders()
  ...
  async function runTask(provider, row, runId) {
    ...
    try {
      const res = await opsApi.runModelTest({
        providerId: provider.id,
        appType: provider.appType,
        model: row.model,
        beta1m: row.beta1m,
  ...
  async function runQueue(tasks, { announce = true } = {}) {
    ...
    try {
      ...
      if (historyResults.length && typeof opsApi?.saveModelTestHistory === 'function') {
        opsApi
          .saveModelTestHistory({
            source: 'manual',
            label:
              tasks.length === 1
                ? `单模型测试：${tasks[0].row.model}`
                : `批量测试（${tasks.length} 项）`,
  ...
  function clearCachedResults() {
    ...
    const ok = window.confirm(`确认清除 ${total} 条测试结果缓存？\n不会改动中转配置与测试范围。`)
    if (!ok) return
  ...
  async function saveCurrentMonitorTargets() {
    ...
    const current = await opsApi.getModelMonitorSettings()
    const settings = current?.settings || {}
    const response = await opsApi.saveModelMonitorSettings({
      ...settings,
      enabled: true,
  ...
  function cancel() {
    ...
    if (cancelToken.runId && typeof opsApi?.cancelModelTestRun === 'function') {
      opsApi.cancelModelTestRun(cancelToken.runId).catch(() => {})
    }
  }

  const { attach: attachKeyboard, detach: detachKeyboard } = useModelTestKeyboard({
    searchQuery,
    showScopeSettings,
