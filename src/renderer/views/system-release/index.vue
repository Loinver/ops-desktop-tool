<template>
  <div class="page">
    ...
    <Teleport to="body">
      <div v-if="showSftpSettings" class="sftp-modal-mask">
        <form
          class="sftp-settings-dialog"
          role="dialog"
          aria-modal="true"
          aria-labelledby="sftp-settings-title"
          @submit.prevent="saveSftpSettings"
        >
          <div class="sftp-dialog-header">
            ...
            <button type="button" class="sftp-icon-button" title="关闭" @click="closeSftpSettings">
              <t-icon name="close" />
            </button>
          </div>

          <div class="sftp-settings-body">
            <p v-if="sftpConfigSource === 'environment'" class="sftp-settings-hint">
            ...
            <div class="sftp-settings-grid">
              <label class="sftp-form-field sftp-field-name">
                ...
                <input
                  v-model.trim="profileSettings.name"
                  type="text"
                  placeholder="例如：生产环境 / 预发布"
                  required
                />
              </label>
              ...
              <label class="sftp-form-field sftp-field-password">
                ...
                <input
                  ...
                  type="password"
                  :placeholder="
                    sftpSettings.hasPassword
                      ? `${sftpSettings.passwordMasked}（留空表示不修改）`
                      : '密码（如使用 SSH Agent 可留空）'
                  "
              ...
              <label
                v-if="profileSettings.healthCheckEnabled"
                class="sftp-form-field sftp-field-local-dir"
              >
                <span>检查地址 <em>*</em></span>
                <input
                  v-model.trim="profileSettings.healthCheckUrl"
                  type="url"
              ...
              <label
                v-if="profileSettings.healthCheckEnabled"
                class="sftp-form-field sftp-field-remote-dir"
              >
                <span>期望状态 / 超时</span>
                <div class="sftp-path-field">
                  <input
                    v-model.number="profileSettings.healthCheckStatus"
                    type="number"
                    min="100"
                    max="599"
                    placeholder="200"
                  />
                  <input
                    v-model.number="profileSettings.healthCheckTimeout"
                    type="number"
                    min="1000"
                    max="60000"
                    placeholder="8000 ms"
              ...
              <label class="sftp-form-field sftp-field-ignore">
                ...
                <textarea
                  v-model="profileSettings.ignoreText"
                  rows="3"
...
*.map"
          ...
          <div class="sftp-dialog-footer">
            <button
              v-if="profileSettings.id && releaseProfiles.length > 1"
              type="button"
              class="sftp-btn-danger"
              @click="removeCurrentProfile"
            >
              删除环境
            </button>
            <span class="sftp-dialog-spacer"></span>
            <button type="button" class="sftp-btn-secondary" @click="closeSftpSettings">
              取消
            </button>
            <button type="submit" class="sftp-btn-primary" :disabled="savingSftpSettings">
    ...
    <Teleport to="body">
      <div v-if="showReleaseHistory" class="sftp-modal-mask">
        ...
        >
          <div class="sftp-dialog-header">
            <div>
              <h3 id="release-history-title">{{ activeReleaseProfileName }} · 发布历史</h3>
              <p>仅显示当前发布环境的记录；成功发布会保留远端备份，可一键回滚。</p>
            </div>
            <button type="button" class="sftp-icon-button" @click="showReleaseHistory = false">
              <t-icon name="close" />
            </button>
          </div>
          <div class="release-history-list">
            <div v-for="item in releaseHistory" :key="item.id" class="release-history-item">
              ...
              <div>
                ...
                <p>{{ item.profileName || '默认环境' }} · {{ item.remoteDir }}</p>
                <small>{{ formatTime(item.finishedAt) }} · {{ item.message }}</small>
              </div>
              <button
                type="button"
                v-if="item.status === 'success' && item.backupPath"
                :disabled="rollingBackId === item.id"
                @click="rollbackRelease(item)"
              >
                {{ rollingBackId === item.id ? '回滚中…' : '一键回滚' }}
              </button>
            </div>
...
const retryableErrors = computed(() =>
  syncErrors.value.filter((err) => err.zipDeploy || (err.localPath && err.remotePath))
)
...
const mergedRows = computed(() => {
  ...
  return sorted.map((name) => {
    ...
    if (local && !remote) status = 'only-local'
      ...
      // 比较大小和修改时间判断是否已修改
      if (local.size !== remote.size || local.modifyTime > (remote.modifyTime || 0)) {
        status = 'modified'
      }
    }
...
function normalizeReleasePaths(paths = {}) {
  ...
  const remoteDir = rawRemoteDir
    ? cleanPath(rawRemoteDir.startsWith('/') ? rawRemoteDir : `/${rawRemoteDir}`)
    : ''
...
async function loadSftpPaths({ showError = false } = {}) {
  if (typeof opsApi?.getSftpPaths !== 'function') {
  ...
  try {
    const result = await opsApi.getSftpPaths()
...
async function saveSftpPaths({ showSuccess = false, paths = null } = {}) {
  ...
  if (typeof opsApi?.saveSftpPaths !== 'function') {
    try {
  ...
  try {
    const result = await opsApi.saveSftpPaths(nextPaths)
...
async function loadReleaseProfiles() {
  if (typeof opsApi?.getSftpProfiles !== 'function') return
  const result = await opsApi.getSftpProfiles()
...
async function switchReleaseProfile() {
  ...
  const result = await opsApi.activateSftpProfile(activeProfileId.value)
  if (!result.success)
    return MessagePlugin.error({ content: result.error, placement: 'bottom-right' })
  applyReleasePaths(result.data)
...
async function openReleaseHistory() {
  ...
  const result = await opsApi.getSftpHistory()
  if (!result.success)
    return MessagePlugin.error({ content: result.error, placement: 'bottom-right' })
...
async function rollbackRelease(item) {
  const confirmed = await opsApi.confirm({
    title: '确认回滚',
    message: `确定回滚“${item.label}”吗？`,
    detail: '将恢复该次发布前的远端文件。'
  })
  if (!confirmed) return
  rollingBackId.value = item.id
  try {
    const result = await opsApi.rollbackSftpRelease(item.id)
...
async function startNewReleaseProfile() {
  ...
  Object.assign(profileSettings, {
    id: '',
    name: `新环境 ${releaseProfiles.value.length + 1}`,
    healthCheckEnabled: false,
    healthCheckUrl: '',
    healthCheckStatus: 200,
    healthCheckTimeout: 8000,
...
async function removeCurrentProfile() {
  const confirmed = await opsApi.confirm({
    title: '删除发布环境',
    message: `确定删除“${profileSettings.name}”吗？`
  })
  if (!confirmed) return
  const result = await opsApi.deleteSftpProfile(profileSettings.id)
  if (!result.success)
    return MessagePlugin.error({ content: result.error, placement: 'bottom-right' })
...
async function openSftpSettings() {
  ...
  const activeProfile = creatingProfile.value
    ? null
    : releaseProfiles.value.find((item) => item.id === activeProfileId.value)
  Object.assign(profileSettings, {
    id: activeProfile?.id || '',
  ...
  try {
    const result = await opsApi.getSftpConfig()
    if (result.success) {
...
async function browseSettingsLocalDir() {
  try {
    const result = await opsApi.browseFile({
      directory: true,
...
async function saveSftpSettings() {
  ...
  try {
    ...
    const result =
      typeof opsApi?.saveSftpProfile === 'function'
        ? await opsApi.saveSftpProfile(profilePayload)
        : await opsApi.saveSftpConfig({
...
async function loadDirectory(dirPath) {
  ...
  loading.value = true
  try {
    const result = await opsApi.sftpList(dirPath)
...
async function loadLocalDir() {
  ...
  try {
    const result = await opsApi.sftpLocalList(requestedLocalDir)
...
async function ensureRemoteDir(remotePath) {
  ...
  const result = await opsApi.sftpList(cleaned)
  if (result && result.success) return

  // 目录不存在，递归创建（sftpMkdir 内部为 recursive）
  const r = await opsApi.sftpMkdir(cleaned)
...
async function uploadFile(localPath, remotePath, fileName) {
  try {
    const result = await opsApi.sftpUpload(localPath, remotePath)
    if (!result.success) {
...
async function deployZip(
  ...
  try {
    const result = await opsApi.sftpDeployZip(payload)
    if (!result.success) {
...
function createUploadSyncTask({ label, localPath, remotePath, successMessage }) {
  return {
    id: nextSyncTaskId++,
    type: 'upload',
    label,
  ...
  try {
    ...
    while (syncQueue.value.length > 0 || ranTask) {
      while (syncQueue.value.length > 0) {
        ...
        try {
          if (task.type === 'zip' && typeof opsApi?.sftpPreflight === 'function') {
            syncMessage.value = `发布前预检: ${task.label}`
            const preflight = await opsApi.sftpPreflight(
...
async function deployFolder(folder) {
  const confirmed = await opsApi.confirm({
    ...
    detail: '只会替换服务器上的同名文件夹；当前目录中的其它文件和文件夹会保留。'
  })
  if (!confirmed) return
...
async function retryUploadTasks(tasks) {
  ...
  for (const task of tasks) {
    ...
    if (enqueueSyncTask(retryTask, { preserveErrors: true })) acceptedTasks.push(task)
  }
...
async function confirmDelete(item) {
  ...
  if (confirmed) {
    try {
      const result = await opsApi.sftpDelete(item.path)
      if (result.success) {
        await refresh()
      } else {
...
onMounted(async () => {
  ...
  try {
    await loadReleaseProfiles()
    await loadSftpPaths()
    const result = await opsApi.sftpTest()
    if (result.success) {
