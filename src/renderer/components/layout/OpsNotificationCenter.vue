    ...
    <transition name="notification-popover">
      <section v-if="open" class="notification-popover" aria-label="全局通知中心">
        <header class="notification-header">
          <div>
            <span class="notification-eyebrow">{{ settingsOpen ? 'NOTIFICATION SETTINGS' : 'OPS NOTIFICATIONS' }}</span>
        ...
        <div v-if="settingsOpen" class="notification-settings">
          <div class="notification-support" :class="{ warning: !notificationSupported }">
            ...
            <span>
              <strong>{{ notificationSupported ? '系统通知可用' : '当前系统不支持桌面通知' }}</strong>
          ...
          <div class="notification-setting-list">
            ...
            <label class="notification-setting-row">
              ...
              <input v-model="preferences.soundEnabled" type="checkbox" :disabled="!preferences.desktopEnabled" />
            </label>
            <label class="notification-setting-row">
              <span><strong>前台也通知</strong><small>应用正在使用时仍显示系统通知</small></span>
              <input v-model="preferences.showWhenFocused" type="checkbox" :disabled="!preferences.desktopEnabled" />
            </label>
            <label class="notification-setting-row">
              <span><strong>恢复通知</strong><small>异常恢复正常后发送一条通知</small></span>
              <input v-model="preferences.notifyRecoveries" type="checkbox" :disabled="!preferences.desktopEnabled" />
            </label>
          </div>
          ...
          <fieldset class="notification-setting-group quiet-settings" :disabled="!preferences.desktopEnabled">
            <legend>免打扰</legend>
            ...
            <div class="quiet-time-row">
              <label>开始<input v-model="preferences.quietHours.start" type="time" :disabled="!preferences.quietHours.enabled" /></label>
              <span>至</span>
              <label>结束<input v-model="preferences.quietHours.end" type="time" :disabled="!preferences.quietHours.enabled" /></label>
            </div>
          </fieldset>

          <label class="notification-repeat-field">
            <span><strong>重复通知间隔</strong><small>相同事件在间隔内只显示一次</small></span>
            <select v-model.number="preferences.repeatIntervalMinutes" :disabled="!preferences.desktopEnabled">
          ...
          <div class="notification-settings-actions">
            ...
            <div>
              <button type="button" :disabled="testingNotification || !notificationSupported" @click="testNotification">
                {{ testingNotification ? '发送中…' : '测试通知' }}
              </button>
              <button class="primary" type="button" :disabled="savingPreferences" @click="savePreferences">
...
async function loadPreferences() {
  ...
  try {
    const result = await window.opsApi?.getOpsNotificationPreferences?.()
...
async function savePreferences() {
  ...
  preferenceError.value = false
  try {
    const result = await window.opsApi?.saveOpsNotificationPreferences?.(plainPreferences())
...
async function testNotification() {
  ...
  preferenceError.value = false
  try {
    const result = await window.opsApi?.testOpsNotification?.()
...
onMounted(() => {
  ...
  unsubscribeNotificationOpen = window.opsApi?.onOpsNotificationOpen?.(handleNativeNotificationOpen)
...
.notification-popover select { -webkit-app-region: no-drag; }
.ops-notification-center { position: relative; }
.notification-trigger { position: relative; width: 36px; height: 36px; display: inline-grid; place-items: center; border: 1px solid var(--shell-border); border-radius: 10px; background: var(--shell-surface-raised); color: var(--text-secondary); font: inherit; font-size: 16px; cursor: pointer; transition: border-color var(--transition-fast), background var(--transition-fast), color var(--transition-fast), box-shadow var(--transition-fast); }
...
.notification-popover { position: absolute; z-index: 65; top: calc(100% + 10px); right: 0; width: min(400px, calc(100vw - 24px)); overflow: hidden; border: 1px solid var(--shell-border); border-radius: 14px; background: var(--shell-surface-raised); box-shadow: var(--shadow-xl); }
...
.notification-header-actions button { min-width: 30px; min-height: 30px; padding: 0 7px; border: 0; border-radius: 8px; background: transparent; color: var(--text-secondary); font: inherit; font-size: 11px; cursor: pointer; }
.notification-header-actions button:has(.t-icon) { width: 30px; padding: 0; font-size: 14px; }
...
.notification-list,
.notification-settings { max-height: min(520px, calc(100vh - 190px)); overflow-y: auto; overscroll-behavior: contain; }
...
.notification-settings { padding: 14px 16px 16px; }
.notification-support { display: flex; align-items: flex-start; gap: 9px; padding: 10px 12px; border-radius: 10px; background: var(--success-light); color: var(--success); }
.notification-support.warning { background: var(--warning-light); color: var(--warning); }
.notification-support > .t-icon { flex: none; margin-top: 2px; }
.notification-support span { min-width: 0; display: grid; gap: 2px; }
.notification-support strong { color: var(--text); font-size: 12px; line-height: 17px; }
.notification-support small { color: var(--text-secondary); font-size: 10px; line-height: 15px; }
.notification-setting-list { margin-top: 10px; }
.notification-setting-row,
.notification-repeat-field { display: flex; align-items: center; justify-content: space-between; gap: 16px; min-height: 48px; padding: 8px 2px; border-bottom: 1px solid var(--border-light); }
.notification-setting-row.compact { min-height: 44px; padding: 4px 0 8px; }
.notification-setting-row > span,
.notification-repeat-field > span { min-width: 0; display: grid; gap: 1px; }
.notification-setting-row strong,
.notification-repeat-field strong { color: var(--text); font-size: 12px; line-height: 17px; }
.notification-setting-row small,
.notification-repeat-field small { color: var(--text-muted); font-size: 10px; line-height: 15px; }
.notification-setting-row > input[type='checkbox'] { width: 16px; height: 16px; flex: none; accent-color: var(--primary); }
.notification-setting-group { min-width: 0; margin: 14px 0 0; padding: 0; border: 0; }
.notification-setting-group legend { margin-bottom: 8px; color: var(--text-muted); font-size: 10px; font-weight: 700; letter-spacing: .08em; }
.notification-option-grid { display: grid; gap: 6px; }
.notification-option-grid.severity-options { grid-template-columns: repeat(3, minmax(0, 1fr)); }
.notification-option-grid.source-options { grid-template-columns: repeat(2, minmax(0, 1fr)); }
.notification-option-grid label { min-width: 0; display: flex; align-items: center; gap: 6px; padding: 7px 8px; border: 1px solid var(--border-light); border-radius: 8px; color: var(--text-secondary); font-size: 11px; cursor: pointer; }
.notification-option-grid label:has(input:checked) { border-color: color-mix(in srgb, var(--primary) 32%, var(--border-light)); background: var(--primary-soft); color: var(--text); }
.notification-option-grid input { width: 14px; height: 14px; flex: none; accent-color: var(--primary); }
.notification-option-grid span { min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.severity-text-critical { color: var(--danger); }
.severity-text-warning { color: var(--warning); }
.severity-text-info { color: var(--info); }
.quiet-settings { padding: 10px 12px 12px; border: 1px solid var(--border-light); border-radius: 10px; background: var(--bg-subtle); }
.quiet-settings legend { padding: 0 4px; margin: 0; }
.quiet-time-row { display: grid; grid-template-columns: minmax(0, 1fr) auto minmax(0, 1fr); align-items: end; gap: 8px; color: var(--text-muted); font-size: 10px; }
.quiet-time-row label { display: grid; gap: 4px; }
.quiet-time-row input,
.notification-repeat-field select { min-width: 0; height: 32px; padding: 0 8px; border: 1px solid var(--border); border-radius: 8px; background: var(--bg-elevated); color: var(--text); font: inherit; font-size: 11px; }
.notification-repeat-field { margin-top: 8px; }
.notification-repeat-field select { width: 92px; flex: none; }
.notification-settings-actions { display: flex; align-items: center; justify-content: space-between; gap: 10px; margin-top: 14px; }
.notification-settings-actions > span { min-width: 0; color: var(--success); font-size: 10px; line-height: 15px; }
.notification-settings-actions > span.error { color: var(--danger); }
.notification-settings-actions > div { display: flex; gap: 7px; margin-left: auto; }
.notification-settings-actions button { min-height: 32px; padding: 0 10px; border: 1px solid var(--border); border-radius: 8px; background: var(--bg-elevated); color: var(--text-secondary); font: inherit; font-size: 11px; cursor: pointer; }
.notification-settings-actions button.primary { border-color: var(--primary); background: var(--primary); color: #fff; }
.notification-settings-actions button:hover:not(:disabled) { border-color: var(--primary); color: var(--primary); }
.notification-settings-actions button.primary:hover:not(:disabled) { color: #fff; background: var(--primary-hover); }
.notification-settings-actions button:disabled,
.notification-setting-group:disabled { opacity: .5; }
...
@media (max-width: 760px) {
  .notification-popover { right: -46px; }
  .notification-settings-actions { align-items: flex-start; flex-direction: column; }
  .notification-settings-actions > div { width: 100%; margin-left: 0; }
  .notification-settings-actions button { flex: 1; }
}
</style>
