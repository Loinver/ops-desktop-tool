        ...
        <div class="palette-body">
          <div v-for="group in visibleGroups" :key="group.name" class="palette-group">
            ...
            <button
              ...
              type="button"
              @mouseenter="activeId = command.id"
              @click="run(command)"
            >
          ...
          <div v-if="visibleCommands.length === 0" class="palette-empty">
            <t-icon name="search" /><strong>没有匹配的命令</strong
            ><span>试试“发布”、“模型”或“快捷启动”。</span>
          </div>
        </div>

        <footer class="palette-footer">
          <span><kbd>↑↓</kbd> 选择</span><span><kbd>↵</kbd> 打开</span
          ><span><kbd>⌘K</kbd> 随时唤起</span>
        </footer>
      </section>
    </div>
  </transition>
    ...
    .map((group) => ({
      ...
      items: group.items.filter(
        (item) =>
          ...
          `${item.name} ${item.description}`.toLowerCase().includes(normalizedQuery.value)
      )
    }))
...
const activeCommand = computed(
  () => visibleCommands.value.find((item) => item.id === activeId.value) || visibleCommands.value[0]
)
...
function moveActive(offset) {
  ...
  const index = Math.max(
    0,
    list.findIndex((item) => item.id === activeCommand.value?.id)
  )
  activeId.value = list[(index + offset + list.length) % list.length].id
}
function onKeydown(event) {
  if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === 'k') {
    event.preventDefault()
    if (props.open) close()
    else emit('open-request')
  }
}
...
watch(
  ...
  async (value) => {
    if (value) {
      query.value = ''
      activeId.value = visibleCommands.value[0]?.id || ''
      await nextTick()
      searchInput.value?.focus()
    }
  }
)
watch(visibleCommands, (list) => {
...
.palette-item.active .palette-icon {
  ...
  color: #fff;
  box-shadow: 0 5px 12px color-mix(in srgb, var(--primary) 32%, transparent);
}
.palette-copy {
  min-width: 0;
  flex: 1;
