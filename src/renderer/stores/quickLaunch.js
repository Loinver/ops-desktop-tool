...
export const useQuickLaunchStore = defineStore('quickLaunch', () => {
  ...
  const quickOpenItems = computed(() => websiteItems.value.filter(item => item.quickOpen === true))
  const filteredItems = computed(() => {
    const query = searchQuery.value.trim().toLowerCase()
    return items.value.filter(item => {
      if (currentTab.value !== 'all' && item.type !== currentTab.value) return false
      if (!query) return true
      return [item.name, item.target, item.type]
  ...
  async function fetchItems() {
    const result = await window.opsApi.getQuickLaunchItems()
  ...
  async function saveItems() {
    return await window.opsApi.saveQuickLaunchItems(toPlainItems(items.value))
  }
  ...
  async function launchItem(item) {
    return await window.opsApi.launchItem(toPlainItem(item))
  }

  async function launchQuickOpenItems() {
    return await window.opsApi.launchQuickLaunchUrls(toPlainItems(quickOpenItems.value))
  }
  ...
  async function importWebsiteItems() {
    return await window.opsApi.importQuickLaunchUrls()
  }

  async function parseWebsiteItems(raw) {
    return await window.opsApi.parseQuickLaunchUrls(raw)
  }
  ...
  async function exportWebsiteItems() {
    return await window.opsApi.exportQuickLaunchUrls(toPlainItems(items.value))
  }

  async function browseFile() {
    return await window.opsApi.browseFile()
  }
